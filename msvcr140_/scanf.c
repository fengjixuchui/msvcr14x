...
void (WINAPI* Real_RtlInitializeBitMap)(PRTL_BITMAP lpBits, PULONG lpBuff, ULONG ulSize);
VOID WINAPI RtlInitializeBitMap(PRTL_BITMAP lpBits, PULONG lpBuff, ULONG ulSize)
{
	if (Real_RtlInitializeBitMap==NULL)
	{
		Real_RtlInitializeBitMap = (void(__stdcall *)(PRTL_BITMAP, PULONG, ULONG))GetProcAddress(GetModuleHandle(TEXT("ntdll.dll")), "RtlInitializeBitMap");
	}
	
	Real_RtlInitializeBitMap(lpBits, lpBuff, ulSize);
}

VOID (WINAPI* Real_RtlSetBits)(PRTL_BITMAP lpBits, ULONG ulStart, ULONG ulCount);
VOID WINAPI RtlSetBits(PRTL_BITMAP lpBits, ULONG ulStart, ULONG ulCount)
{
	if (Real_RtlSetBits == NULL)
	{
		Real_RtlSetBits = (void(__stdcall *)(PRTL_BITMAP, ULONG, ULONG))GetProcAddress(GetModuleHandle(TEXT("ntdll.dll")), "RtlSetBits");
	}
	Real_RtlSetBits(lpBits, ulStart, ulCount);
}

BOOLEAN (WINAPI* Real_RtlAreBitsSet)(PCRTL_BITMAP lpBits, ULONG ulStart, ULONG ulCount);
BOOLEAN WINAPI RtlAreBitsSet(PCRTL_BITMAP lpBits, ULONG ulStart, ULONG ulCount)
{
	if (Real_RtlAreBitsSet==NULL)
	{
		Real_RtlAreBitsSet= (BOOLEAN(__stdcall *)(PCRTL_BITMAP, ULONG, ULONG))GetProcAddress(GetModuleHandle(TEXT("ntdll.dll")), "RtlAreBitsSet");
	}
	return Real_RtlAreBitsSet(lpBits, ulStart, ulCount);
}

BOOLEAN (WINAPI* Real_RtlAreBitsClear)(PCRTL_BITMAP lpBits, ULONG ulStart, ULONG ulCount);
BOOLEAN WINAPI RtlAreBitsClear(PCRTL_BITMAP lpBits, ULONG ulStart, ULONG ulCount)
{
	if (Real_RtlAreBitsClear == NULL)
	{
		Real_RtlAreBitsClear = (BOOLEAN(__stdcall *)(PCRTL_BITMAP, ULONG, ULONG))GetProcAddress(GetModuleHandle(TEXT("ntdll.dll")), "RtlAreBitsSet");
	}
	return Real_RtlAreBitsClear(lpBits, ulStart, ulCount);
}
 ...
 *		_snscanf (MSVCRT.@)
 */
int WINAPIV _snscanf(char *input, size_t length, const char *format, ...)
{
    va_list valist;
    int res;
 ...
 *		_snscanf_l (MSVCRT.@)
 */
int WINAPIV _snscanf_l(char *input, size_t length,
 ...
 *		_snscanf_s (MSVCRT.@)
 */
int WINAPIV _snscanf_s(char *input, size_t length, const char *format, ...)
{
    va_list valist;
    int res;
 ...
 *		_snscanf_s_l (MSVCRT.@)
 */
int WINAPIV _snscanf_s_l(char *input, size_t length,
        const char *format, _locale_t locale, ...)
{
    va_list valist;
    int res;
                                       ...
                                       va_list valist)
{
     ...
     * scanf. LEGACY_COMPATIBILITY affects parsing of nan/inf,
 ...
 *		vsscanf (MSVCRT120.@)
 */
int CDECL vsscanf(const char *buffer, const char *format, va_list valist)
{
    if (!CHECK_PMT(buffer != NULL && format != NULL)) return -1;
...
int __cdecl __conio_common_vcscanf(
...
{
	return vcscanf_l(format, locale, arglist);
}
...
{
	return vcwscanf_l(format, locale, arglist);
}