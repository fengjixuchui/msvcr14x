...
static inline int FUNC_NAME(pf_output_wstr)(FUNC_NAME(puts_clbk) pf_puts, void *puts_ctx,
        const wchar_t *str, int len, pthreadlocinfo locinfo)
{
#ifdef PRINTF_WIDE
    ...
    LPSTR out;
    BOOL def_char;
    int len_a = WideCharToMultiByte(locinfo->_public._locale_lc_codepage, WC_NO_BEST_FIT_CHARS,
    ...
    WideCharToMultiByte(locinfo->_public._locale_lc_codepage, WC_NO_BEST_FIT_CHARS,
...
static inline int FUNC_NAME(pf_output_str)(FUNC_NAME(puts_clbk) pf_puts, void *puts_ctx,
        const char *str, int len, pthreadlocinfo locinfo)
{
#ifdef PRINTF_WIDE
    LPWSTR out;
    int len_w = MultiByteToWideChar(locinfo->_public._locale_lc_codepage, 0, str, len, NULL, 0);

    out = HeapAlloc(GetProcessHeap(), 0, len_w*sizeof(WCHAR));
    if(!out)
        return -1;

    MultiByteToWideChar(locinfo->_public._locale_lc_codepage, 0, str, len, out, len_w);
...
static inline int FUNC_NAME(pf_output_format_wstr)(FUNC_NAME(puts_clbk) pf_puts, void *puts_ctx,
        const wchar_t *str, int len, FUNC_NAME(pf_flags) *flags, pthreadlocinfo locinfo)
{
    int r, ret;
...
static inline int FUNC_NAME(pf_output_format_str)(FUNC_NAME(puts_clbk) pf_puts, void *puts_ctx,
        const char *str, int len, FUNC_NAME(pf_flags) *flags, pthreadlocinfo locinfo)
{
    int r, ret;
...
static inline int FUNC_NAME(pf_handle_string)(FUNC_NAME(puts_clbk) pf_puts, void *puts_ctx,
        const void *str, int len, FUNC_NAME(pf_flags) *flags, pthreadlocinfo locinfo, BOOL legacy_wide)
{
...
#else
        BOOL two_digit_exp = (_get_output_format() == _TWO_DIGIT_EXPONENT);
#endif
        ...
        args_clbk pf_args, void *args_ctx, va_list *valist)
{
    ...
    BOOL positional_params = options & PRINTF_POSITIONAL_PARAMS;
    BOOL invoke_invalid_param_handler = options & PRINTF_INVOKE_INVALID_PARAM_HANDLER;
#if _MSVCR_VER >= 140
    ...
    while(*p) {
        ...
        if(flags.Format == 's' || flags.Format == 'S') {
            ...
            if(!n_format_enabled) {
                INVALID_PMT("\'n\' format specifier disabled", EINVAL);
                return -1;
            }
...
int FUNC_NAME(create_positional_ctx)(void *args_ctx, const APICHAR *format, va_list valist)
{
    ...
    i = FUNC_NAME(pf_printf)(FUNC_NAME(puts_clbk_str), &puts_ctx, format, NULL,
            PRINTF_POSITIONAL_PARAMS, arg_clbk_type, args_ctx, NULL);
    if(i < 0)
        return i;
